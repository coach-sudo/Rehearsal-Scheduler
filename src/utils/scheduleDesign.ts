...
function baseCss(state: AppState, design: ScheduleDesignSettings, fonts: { heading: string; body: string }) {
  ...
  const baseStyles = `
    ...
    footer,.schedule-footer{position:relative;flex:0 0 auto;min-height:0;border-top:1px solid ${dark ? "#334155" : "#d7d7d2"};margin-top:${onePage ? "4px" : "16px"};padding-top:${onePage ? "3px" : "8px"};font-size:${microText}px;opacity:.72;break-inside:avoid}.schedule-footer>span{display:block}
