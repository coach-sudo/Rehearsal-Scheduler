  ...
  return `<section class="weekly-engine ${hasCallKey ? "has-call-key" : ""}" style="${typographyVariables(design)};--day-count:${Math.max(1, dates.length)}">
    <div class="pro-grid">
      ...
      <div class="grid-time-axis">
        ${ticks.map((tick) => `<span class="grid-time-marker" style="top:${((tick - start) / total) * 100}%">${escapeHtml(formatScheduleTime(minutesToTime(tick), design))}</span>`).join("")}
      </div>
...
function baseCss(state: AppState, design: ScheduleDesignSettings, fonts: { heading: string; body: string }) {
  ...
  return `
    ...
    .schedule-content{flex:1 1 auto;min-height:0;overflow:hidden;position:relative}
    .scaled-content{width:var(--content-width);height:calc(100% / var(--content-scale));transform:scale(var(--content-scale));transform-origin:top left}
