...
function renderHeader(state: AppState, design: ScheduleDesignSettings, page?: RenderPagePlan) {
  ...
  const directorLine = [design.directorName, design.directorContact].filter(Boolean).join(" | ");
  return `<header class="header ${design.headerStyle}">
    ...
    <div class="header-copy">
      ...
      ${design.directorContactPlacement === "header" && directorLine ? `<div class="notes director-contact">${escapeHtml(directorLine)}</div>` : ""}
...
function renderFooter(design: ScheduleDesignSettings) {
  ...
  const footerParts = [
    design.footerText || "Generated by Rehearsal Scheduler",
    design.directorContactPlacement === "footer" ? directorLine : "",
  ].filter(Boolean);
  return `<footer class="schedule-footer">
    <span>${escapeHtml(footerParts.join(" | "))}</span>
...
function typographyVariables(design: ScheduleDesignSettings) {
  const cellContentAlign = design.cellContentVerticalAlign === "center" ? "center" : design.cellContentVerticalAlign === "bottom" ? "flex-end" : "flex-start";
  return `--time-align:${design.timeTextAlign};--work-align:${design.workTextAlign};--actor-align:${design.actorTextAlign};--time-weight:${design.boldTimes ? 850 : 500};--work-weight:${design.boldWork ? 850 : 500};--actor-weight:${design.boldActorNames ? 850 : 400};--cell-content-align:${cellContentAlign}`;
}
...
function baseCss(state: AppState, design: ScheduleDesignSettings, fonts: { heading: string; body: string }) {
  ...
  const densityPad = `${Math.round(2 + densityRatio * 6)}px ${Math.round(3 + densityRatio * 8)}px`;
  const gridHeaderHeight = `${Math.round(30 + densityRatio * 18)}px`;
  ...
  const gridDetailSize = Math.max(7.2, Math.min(10.5, minText - 1.35 + densityRatio * 1.1));
  ...
  const weeklyGridStyles = `
    ...
    .grid-call-block{display:flex;flex-direction:column;justify-content:var(--cell-content-align,flex-start);padding:${densityPad};overflow:hidden;line-height:1.1;contain:layout paint}
    .grid-call-block strong{padding-right:15px;font-size:${Math.max(8, Math.min(11.5, minText - 1 + densityRatio * 1.2))}px;line-height:1.05;overflow-wrap:anywhere}
    ...
    .grid-call-room{font-size:${gridDetailSize}px;text-align:var(--actor-align,left);font-weight:750;opacity:.76;white-space:normal;overflow-wrap:anywhere}
    .grid-call-actors,.grid-call-detail{margin-top:${Math.round(1 + densityRatio * 2)}px;font-size:${gridDetailSize}px;line-height:1.08;text-align:var(--actor-align,left);overflow-wrap:anywhere}
